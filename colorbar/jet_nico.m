function c = jet_nico_br(m)
%jet_nico_br    Shades of dark red and blue color map
%   jet_nico_br(M), is an M-by-3 matrix that defines a colormap.
%   The colors begin with dark blue, range through shades of
%   blue to green, and then through shades of red to dark red.
%   jet_nico_br, by itself, is the same length as the current figure's
%   colormap. If no figure exists, MATLAB creates one.
%
%   For example, to reset the colormap of the current figure:
%
%             colormap(jet_nico_br)
%
...
% From [0.85 0.65 0.80] to [0.44 0.19 0.62 ] to [0.16 0.20 0.43] to [0.32 0.78 0.94 ] to [0.3 1 0.6], then [0.3 1 0.6] to [0.96 0.92 0.23] to [0.89 0.22 0.18] to [0.2 0 0] to [0.80 0.60 0.40]
% ----------pink-------------deep purple------------darkblue--------------azur------------green--  -----green-----dark yellow----------red------------dark red-------light brown-----
%if (mod(m,2) == 0)
%    m0 = m*0.5;
%else
%    m0 = floor(m*0.5);
...
m1 = floor(m0*0.25);
if ( m1 > 1 ),
  r1 = ((0:m1-1)'/max(m1-1,1))*(0.44-0.85)+0.85;
  g1 = ((0:m1-1)'/max(m1-1,1))*(0.19-0.65)+0.65;
  b1 = ((0:m1-1)'/max(m1-1,1))*(0.62-0.80)+0.80;
elseif ( m1 == 1 ),
  r1 = 0.85;
  g1 = 0.65;
  b1 = 0.80;
...
% dark blue to azur :
m3 = floor(m0*0.75)-numel(r1)-numel(r2);
if ( m3 > 1 ),
...
r4 = ((1:m4)'/max(m4,1))*(0.30-0.32)+0.32;
g4 = ((1:m4)'/max(m4,1))*(1.00-0.78)+0.78;
b4 = ((1:m4)'/max(m4,1))*(0.60-0.94)+0.94;
% green to dark yellow :
m5 = ceil(m0*0.25);
if ( m5 > 1 ),
  r5 = ((0:m5-1)'/max(m5-1,1))*(0.96-0.30)+0.30;
  g5 = ((0:m5-1)'/max(m5-1,1))*(0.92-1.00)+1.00;
  b5 = ((0:m5-1)'/max(m5-1,1))*(0.23-0.60)+0.60;
elseif ( m5 == 1 ),
  r5 = 1.00;
  g5 = 0.90;
  b5 = 0.17;
